...
function createReferenceCode(): string {
  return `MM${Date.now().toString(36)}${Math.random().toString(36).slice(2,6)}`.toUpperCase();
}
...
export const createTopUpIntent = asyncHandler(async (req: Request, res: Response) => {
  ...
  // NOTE: QR image: either a fixed QR image for company account or generate/return URL to image hosting
  const qrImageUrl = process.env.TOPUP_QR_IMAGE_URL || 'https://cdn.mentorme.vn/qr/company_account.png';
...
export const confirmTopUpTransferred = asyncHandler(async (req: Request, res: Response) => {
  const userId = (req as any).user?.id;
  if (!userId) return forbidden(res, 'Unauthorized');
  ...
  if (intent.status == 'SUBMITTED') {
    return ok(res, { id: intentId, status: intent.status, idempotent: true }, 'Already submitted or processed');
  }
  ...
  return ok(res, { id: intent._id, status: intent.status }, 'Marked as submitted');
});
...
export const getMyTopUpIntents = asyncHandler(async (req, res) => {
  ...
  const items = await TopUpIntent.find({ user: userId })
    .sort({ createdAt: -1 });
...
// Admin endpoints
export const listPendingTopUpsAdmin = asyncHandler(async (req: Request, res: Response) => {
  const role = (req as any).user?.role;
  if (!role || !['admin','root'].includes(role)) return forbidden(res, 'Unauthorized');

  const items = await TopUpIntent.find({ status: { $in: ['PENDING','SUBMITTED'] } }).sort({ createdAt: -1 }).populate('user','userName email');
  return ok(res, { items }, 'OK');
});

export const approveTopUpAdmin = asyncHandler(async (req: Request, res: Response) => {
  const role = (req as any).user?.role;
  if (!role || !['admin','root'].includes(role)) return forbidden(res, 'Unauthorized');
  ...
  try {
    ...
    if (!intent) { await session.abortTransaction(); return notFound(res, 'Not found'); }
    ...
    let wallet = await Wallet.findOne({ userId: intent.user }).session(session);
    if (!wallet) {
      wallet = new Wallet({ userId: intent.user, currency: intent.currency ?? 'VND', balanceMinor: 0, status: 'ACTIVE' });
      await wallet.save({ session });
    }
    if (wallet.status !== 'ACTIVE') { await session.abortTransaction(); session.endSession(); return badRequest(res, 'WALLET_LOCKED'); }
    ...
    return ok(res, { id: intent._id, status: intent.status, tx: tx[0] }, 'Approved');
  } catch (err) {
...
export const rejectTopUpAdmin = asyncHandler(async (req: Request, res: Response) => {
  const role = (req as any).user?.role;
  if (!role || !['admin','root'].includes(role)) return forbidden(res, 'Unauthorized');
  ...
  if (intent.status === 'APPROVED') return badRequest(res, 'Already approved');
  ...
  await sendPushToUser(String(intent.user), {
    ...
    body: `Yêu cầu nạp ${intent.amount} VND đã bị từ chối${note ? `: ${note}` : ''}`,
